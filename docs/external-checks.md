...
The earlier [NetCore check](independent-check.md) is useful too, but NetCore is my own project. I later ran the same lost-response check against [20 small FastAPI repos](compatibility-checks.md) and [27 Flask, Express, and Spring repos](compatibility-checks-2.md). None of these runs proves RecoveryLab works with every HTTP service or every failure mode.
